  ...
  return <div className="min-h-screen bg-background">
      ...
      <article>
        ...
        <div ref={contentRef as React.RefObject<HTMLDivElement>} className={`max-w-4xl mx-auto px-4 py-16 transition-all duration-1000 ${contentVisible ? 'opacity-100 blur-0' : 'opacity-0 blur-sm'}`}>
          ...
          {tableOfContents.length > 0 && <div className="bg-gradient-to-br from-brand-50 to-white border-l-4 border-primary rounded-lg p-8 mb-12 shadow-sm">
              ...
              <nav>
                <ul className="space-y-3">
                  {tableOfContents.map((item, index) => <li key={item.id} className={`${item.level === 3 ? 'ml-8' : ''} transition-all duration-200`}>
                      <a href={`#${item.id}`} onClick={e => handleTocClick(e, item.id)} className={`${item.level === 2 ? 'text-base font-semibold text-foreground' : 'text-sm text-muted-foreground'} hover:text-primary transition-colors hover:underline flex items-start gap-2 group`}>
          ...
          <div className="prose prose-lg max-w-none
              ...
              prose-img:rounded-lg prose-img:shadow-md prose-img:my-8" dangerouslySetInnerHTML={{
          ...
          {/* Tags */}
          {post.tags && post.tags.length > 0 && <div className="mt-12 pt-8 border-t border-border">
